...
GROUPS: Dict[str, List[str]] = {
    ...
    # 本番に足すと PR-AUC 0.257 → 0.275。探索込み・5モデルでも分離力の改善は
    # 再現し、運用の規則（ブースティング3モデルが揃って上位）での正例率は
    # 28.7% → 30.5%（実験27・27b、docs/MODEL_DISCLOSURE_TIMING.md）。
...
#: 本番で使うプリセット。**ここが正本。**
#:
#: 2026-09-22、実験39 で 153列 -> 205列 に切り替えた（運用者の決定）。
#: 3モデルとも足切り（PR-AUC の差 > 0.0048）を超えた:
#:   lgbm 0.2735 -> 0.2818 (+0.0083)
