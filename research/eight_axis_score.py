...
ただし軸8（進捗期待）は、2026-09-25 に画面が「前年同期の進捗で割った比率の順位」
（src/lib/scoring.js の scoreProgress）に変えた後も、ここは旧式
（5 + (進捗率 − Q×25)/2）のまま。データセットに前年同期の基準の列が無いため。
いまはどこからも呼ばれていない（ベースラインの検証は済んでいる）。
"""
