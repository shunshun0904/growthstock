...
def write_md(rep: Dict, path: str) -> None:
    ...
    for name, h in rep["headline"].items():
        b1, b2 = h["best_per_trade"], h["best_per_month"]
        body.append([
            name,
    ...
    A(_t(["候補の選び方", "1件あたりが最大の出口", "時間あたりが最大の出口",
          "60日以内の最大値（後知恵）", "最大値が付く日の中央値"], body))
    A("")
    ...
    for sec in rep["sections"]:
        ...
        A(_t(["出口", "件数", "平均", "中央値", "勝率", "下側5%",
              "平均保有日数", "1ヶ月換算", "到達率"], body))
        A("")
        pk = sec["peak60"]
        ...
        if sec.get("by_period"):
            ...
            head = ["期間", "件数"] + [f"{k}日" for k in ks]
            rows = []
            for pr in sec["by_period"]:
                by = {r["k"]: r for r in pr["rows"]}
                rows.append([pr["period"], f"{pr['n']:,}"]
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print("\n[eval] 部分集合ごとに出口を比べる")
    sections = evaluate_subsets(panel, ev, args.fee, args.test_start)
    ...
    for name, h in rep["headline"].items():
        b1 = h["best_per_trade"]
        print(f"  {name}: 1件あたり最大 = {b1['rule'] if b1 else '—'}")
    return 0
