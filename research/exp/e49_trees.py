#!/usr/bin/env python3
"""
実験49: LightGBM の木の本数を 200 と 500 で、同じデータで3通り探索して比べる。

運用者の指示（2026-09-25）「どうせならn_estimatorの数を500に変更して下さい。特徴量も
増えてきたので。」を受けて本番の探索を一度 500 にしたときに作った。その後、追加モデルまで
500本にすると週次のジョブに収まらない見込みと分かり、運用者の判断で全モデル200本のままに
した（docs/MODEL_ADOPTION_RULES.md §12）。500本が効くかの参考として残す。見るもの:
...
2. 学習率が探索範囲の端に張り付かないか。200本では 0.016〜0.024 が選ばれてきた
   ...
   以前の範囲 0.01〜0.2 の下限より下。範囲を本数に合わせる作り（tuning.LR_TOTAL。500本で
   0.004〜0.08）の腕（B）と、動かさない腕（C）を並べて、動かした効果を見る
...
--cv year_cap_date。違うのは木の本数と学習率の範囲だけ）
  A  木200本・学習率 0.01〜0.2    週次実行と同じ（9/27 もこれ。実験48 の N と同じ作り）
  B  木500本・学習率 0.004〜0.08  本数に合わせて範囲を動かす
...
LABELS = {"A": "A 木200本・学習率 0.01〜0.2（週次と同じ）",
          "B": "B 木500本・学習率 0.004〜0.08（範囲も合わせる）",
