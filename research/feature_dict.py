...
MACRO: List[tuple] = [
    ...
    ("ボラの分解", ["vol_factors"],
     "vol_20d（値動きの荒さ）を『どんな種類の荒さか』に分けた比。自分の平常・市場・業種との比、"
     "上方÷下方、直前5日÷20日、寄り付きギャップ÷日中レンジ。水準は vol_20d が持つ（実験51）"),
    ("本の第2・3章の抜け", ["fund_book"],
     "取得済みの J-Quants から作れるのに特徴量にしていなかったもの（現金・投資CF・財務CF・CFの増減・持続可能成長率・株主資本回転率・増配・BPS と株数の変化・会計基準）"),
    ("上場からの年数", ["listing"],
     "一般市場に上場（TOKYO PRO MARKET から移行）してからの年数。J-Quants では "
     "2016-10 より前が見えないので5年で打ち止めにした"),
]
...
COL_JA = {
    ...
    "vol_rel_long": "vol_20d ÷ 直近120営業日のボラ。いま異常に荒いのか、もともと荒い銘柄なのか",
    "vol_rel_mkt": "vol_20d ÷ TOPIX の20日ボラ。相場全体が荒いのか、この銘柄だけが荒いのか",
    "vol_rel_sector": "vol_20d ÷ その銘柄の業種指数の20日ボラ",
    "vol_updown": "上昇日の半分散の平方根 ÷ 下落日の同じもの（20日）。上に跳ねて荒いのか、投げられて荒いのか",
    "vol_rel_short": "直近5日のボラ ÷ vol_20d。ブレイク直前に膨らんだのか、落ち着いてきたのか",
    "vol_gap_ratio": "寄り付きギャップ（始値÷前日終値−1）の σ ÷ 日中レンジ（log 高値÷安値）の σ（20日）。材料で飛んだのか、日中の売買で動いたのか",
    "cash_mcap": "直近に開示された現金同等物 ÷ 時価総額（%）。CashEq は2Q・通期にしか無いので直近3開示まで前に埋める",
    "cfi_mcap": "投資CF（開示時点の累計）÷ 時価総額（%）。負なら投資している",
    "cff_mcap": "財務CF（開示時点の累計）÷ 時価総額（%）。正なら調達、負なら返済・還元",
    "cfo_yoy_sym": "営業CF累計の前年同期比（対称、−100〜100）",
    "fcf_yoy_sym": "フリーCF累計の前年同期比（対称）",
    "sustainable_growth": "サスティナブル成長率 = ROE × (1 − 配当性向)（%）",
    "equity_turnover": "株主資本回転率 = 売上TTM ÷ 自己資本",
    "div_growth_sym": "1株配当（会社予想を優先）の前年同期比（対称）",
    "div_up": "増配（前年同期より1株配当が多い）なら1",
    "bps_yoy": "BPS の前年同期比（%）",
    "shares_yoy": "自己株控除後の株数の前年同期比（%）。増資なら正、自社株買い・消却なら負",
    "acct_ifrs": "会計基準が IFRS / US / 外国なら1、日本基準なら0（DocType から）",
    "cash_chg1_sym": "現金同等物の、直近の開示と1回前の開示の変化（対称、−100〜100）",
    "cash_chg2_sym": "現金同等物の、1回前と2回前の開示の変化（対称）",
    "cfo_yoy_sym_p1": "営業CF累計の、1年前の前年同期比（2年前 → 1年前、対称）",
    "fcf_yoy_sym_p1": "フリーCF累計の、1年前の前年同期比（対称）",
    "cfi_yoy_sym": "投資CF累計の前年同期比（対称）",
    "cfi_yoy_sym_p1": "投資CF累計の、1年前の前年同期比（対称）",
    "cff_yoy_sym": "財務CF累計の前年同期比（対称）",
    "cff_yoy_sym_p1": "財務CF累計の、1年前の前年同期比（対称）",
    "sustainable_growth_chg1": "サスティナブル成長率の、前回開示からの変化（pt）",
    "sustainable_growth_chg2": "サスティナブル成長率の、2回前 → 前回の変化（pt）",
    "equity_turnover_chg1": "株主資本回転率の、前回開示からの変化",
    "equity_turnover_chg2": "株主資本回転率の、2回前 → 前回の変化",
    "div_growth_sym_p1": "1株配当の、1年前の前年同期比（対称）",
    "bps_yoy_p1": "BPS の、1年前の前年同期比（%）",
    "shares_yoy_p1": "株数の、1年前の前年同期比（%）",
    ...
    "listing_years": "一般市場に上場（TOKYO PRO MARKET から移行）してからの年数。5年で打ち止め。"
