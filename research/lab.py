...
#: 5営業日平均終値で売る。
#:
#: **h=20（約1ヶ月）にした（2026-09-17）。目的変数の RISE_HORIZON と揃える。**
#:
#: 物差しは「実際に売る日」に合わせるのが筋で、そこを外すと、最適化している
#: ものと測っているものが別になる。目的変数を20営業日に短くしたので、
#: 評価も20営業日で測る。
#:
#: **ただし、40営業日のほうが数字は良く出ていた**（h=60 のラベルで学習した
#: モデルでの実測。この表は残しておく）:
#:
#:   保有      優位      窓平均    t値    最悪の窓
#:   20営業日  +0.78pt  +0.43pt  +1.66  -0.90pt
#:   40営業日  +1.18pt  +0.91pt  +3.06  -0.41pt
#:   60営業日  +1.20pt  +0.89pt  +1.74  -1.67pt
#:   120営業日 +2.78pt  -1.73pt  -0.80 -16.78pt
#:
#: 40日が良く見えたのは「40日持てば優位が積み上がる」ためで、20日で
#: 降りる運用にその数字を当てはめることはできない。持たない期間の利益を
#: 自分の成績に数えることになる。良く見える数字より、実際に手に入る数字を測る。
#:
#: なお上の表は **h=60 のラベルで学習したモデル**のもの。いまは h=20 の
#: ラベルで学習するので、20営業日の数字は変わりうる。新旧ラベルを同じ窓で
#: 学習して両方の物差しで測った結果は docs/MODEL_HORIZON.md にある。
#:
#: 20/40/60/120 営業日を全部持たせてあるので、outcome= で切り替えられる。
#: 過去の記録（ノイズ床・ETF実験・ウォークフォワード）はすべて ret_o1_40 で
#: 測ってあるので、そのまま比べられない点に注意。
OUTCOME = "ret_o1_20"
...
def frame(rebuild: bool = False) -> pd.DataFrame:
    ...
    2種類を持たせる。
      ref_end / ref_rise  ラベル定義に依存しない従来の物差し
                          ...
                          sweep_design.REF_HORIZON=60 で固定してあり、
                          RISE_HORIZON を動かしても動かない（動かすと
                          過去の掃引と比べられなくなるため）
