...
#: 5営業日平均終値で売る。
#:
#: h=40（約2ヶ月）にしてある。実測で60営業日より優れていた。
#:
#:   保有      優位      窓平均    t値    最悪の窓
...
#:
#: 優位は60日とほぼ同じなのに、t値が3.06と倍近く、最悪の窓も4分の1。
#: 選んだ銘柄は40日で60日ぶんの利益の69%を実現しており、残り31%のために
#: 期間を1.5倍にすると、その間に市場全体のノイズが積み上がる。ノイズは
#: 同じ窓の全候補に共通で乗るので、窓ごとの「差」が測りにくくなる。
#: 優位は変わらず分散だけ増える。120日では優位そのものが消える。
#:
