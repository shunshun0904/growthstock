...
この文書の対象は「入れ替わったもの」である。

**結論（2026-09-12）: 20日化は採用しない。** 「60日では正例 → 20日では負例」が
2,608件（11.4%）と大きく、その群は3ヶ月では +24.2%（中央値）上がっている。
最初の1ヶ月で動かないという理由だけで捨てるには多すぎる、という判断。
本番のラベル（build_dataset.py の DEFAULT_RISE）は変更していない。
ここにあるのは比較専用の設定であり、再検討したくなったときに同じ集計を
引き直せるように残してある。
"""
