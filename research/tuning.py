...
#:
#: 2026-09-25 に一度 500 にした（fcbe4c1。運用者の指示「どうせならn_estimatorの数を
#: 500に変更して下さい。特徴量も増えてきたので」）が、追加モデル（xgb / cat）まで500本に
#: すると週次のジョブ（上限330分）に収まらない見込みと分かり、運用者の判断で全モデル
#: 200 のままにした（「であれば、lgbmも200のままでよいです」。docs/MODEL_ADOPTION_RULES.md §12）。
#:
#: 追加モデルもこの値を使う（tuning_multi.N_ESTIMATORS）。変えるときは週次のジョブの
#: 時間の見積り（§12）を先に確かめる。学習率の探索範囲は本数に合わせて動き（LR_TOTAL）、
#: 本番の学習（train_production.py）は探索したときの本数がこの値と違えば止まる。
SEARCH_N_ESTIMATORS = 200

#: 学習率の探索範囲を「学習率 × 木の本数」（1本ごとの歩幅の合計）で決める。
#:
#: 木200本のときの範囲 0.01〜0.2 は、歩幅の合計で 2〜40。本数を変えても同じ合計の範囲を
#: 探すように、学習率の範囲を本数で割って決める（200本なら従来どおり 0.01〜0.2、
#: 500本なら 0.004〜0.08）。0.01〜0.2 のまま500本にすると、200本の探索で選ばれてきた
#: 学習率 0.016〜0.024（歩幅の合計 3.2〜4.8。lgbm_params.json と docs/MODEL_HORIZON.md の
#: 記録）に当たる 0.0063〜0.0097 が範囲の下限より下になり、探せなくなる、と考えた。
#: ただし実験49 では500本で選ばれた学習率は 0.015 前後（歩幅の合計 6.8〜7.5）で、どちらの
#: 範囲にも入っていた（docs/MODEL_ADOPTION_RULES.md §12）。200本では何も変えない備えとして残す。
LR_TOTAL = (2.0, 40.0)
