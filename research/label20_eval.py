...
def write_md(rep: Dict, path: str) -> None:
    ...
    A("## 判断（2026-09-12）")
    A("")
    A("**定義は変えない。現行の60営業日のまま運用する。**")
    A("")
    A("「60日では正例 → 20日では負例」が全体の11.4%にあたる2,608件と大きい。"
      "この群は3ヶ月では +24.2%（中央値）上がっており、"
      "最初の1ヶ月で動かないという理由だけで捨てるには多すぎる。")
    A("")
    A("**残る食い違いは把握したうえでの判断である。** "
      "出口の実測（[docs/EXIT_TIMING.md](EXIT_TIMING.md)）では TOPIX 超過は"
      "20〜30営業日で尽きる。60日ラベルを使い続ける限り、"
      "モデルが選ぶもの（3ヶ月のどこかで上がる）と、"
      "実際に超過が取れる期間（約1ヶ月）はずれたままになる。"
      "遅れて上がる群まで取りに行くなら保有も延ばすことになり、"
      "そのぶん地合いの分がリターンの大半を占める。")
    A("")
    A("以下はその判断の材料。**本番のラベル（`build_dataset.py`）は変更していない。**"
      "この集計は比較専用の設定（`research/label20.py`）で引き直したもの。")
    A("")
    A("## 何を変えたら何が起きるか")
    A("")
    A(_t(["", "現行", "新案"],
