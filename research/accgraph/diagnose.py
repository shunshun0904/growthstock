...
def verdict(res: Dict) -> Tuple[str, str]:
    ...
    if straddle and diff_neg:
        return "群の性質", ("全体で学習しても明細ありの群は 0.5 と区別できず、"
                          ...
                          "予測しにくい")
    # ここから下は事前の基準どおり「判定できない」。理由だけを型ごとに書き分ける。
    # 当初は一律に「件数が足りない」と書いていたが、実測で「0.5 は上回るが
    # 揃えた群より有意に低い」型が出た。これは区間が広いのではなく、
    # 2つの基準が想定していなかった混合の形なので、そう書く
    if above and diff_neg:
        return "判定できない", ("0.5 は上回るが、流動性を揃えた群より有意に低い。"
                             "事前に決めた2つの型のどちらでもなく、件数と群の"
                             "性質の両方が効いている形")
    if ed[1] < 0.5:
        return "判定できない", "明細ありの群の AUC が 0.5 を有意に下回る"
    if df[0] > 0.0:
        return "判定できない", "明細ありの群のほうが流動性を揃えた群より有意に高い"
