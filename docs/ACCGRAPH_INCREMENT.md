...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| logit | compact（72列） | 1,920 | 0.5357 [0.516, 0.555] | 0.5332 [0.512, 0.554] | 0.5328 [0.511, 0.553] | -0.0004 [-0.002, +0.002] | -0.0011〜+0.0021 | **差が見えない** |
| logit | full（202列） | 1,920 | 0.5357 [0.516, 0.555] | 0.5332 [0.512, 0.554] | 0.5334 [0.511, 0.554] | +0.0002 [-0.004, +0.004] | — | （参考）差が見えない |
...
- `logit/compact`: **差が見えない** — 差の95%区間が0をまたぐ。上積みがあるとしても +0.002 程度まで
- `logit/full`: **差が見えない** — 差の95%区間が0をまたぐ。上積みがあるとしても +0.004 程度まで
- `lgbm/compact`: **差が見えない** — 差の95%区間が0をまたぐ。上積みがあるとしても +0.002 程度まで
- `lgbm/full`: **差が見えない** — 差の95%区間が0をまたぐ。上積みがあるとしても +0.004 程度まで

## 較正し直しの効果（参考）

| 1段目 | 明細の特徴量 | 明細なしの2段目 − 1段目のまま |
| --- | --- | ---: |
| logit | compact | -0.0025 [-0.009, +0.003] |
| logit | full | -0.0025 [-0.009, +0.003] |
...
- 差が見えないときは、区間の上限を見る。上限が小さければ、少なくともそれより大きな上積みは今の件数でも否定できている。
