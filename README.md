...
**2026-09-23 に結論をまとめて区切りました**（[`docs/ACCGRAPH_REPORT.md`](docs/ACCGRAPH_REPORT.md)）。
会計情報だけで AUC 0.558 前後まではわずかに予測できますが、コスト控除後の損益はほぼマイナスで、
GNN（GraphSAGE + GRU）は平らな特徴量の LightGBM に有意に負けました（AUC の差 −0.0098、95%区間 [−0.0149, −0.0047]）。
EDINET の明細も、取得済みの106社の範囲では上積みがありませんでした。
