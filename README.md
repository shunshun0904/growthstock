...
| 8 | 進捗期待 | 決算進捗率 vs 前年同期の進捗 | 比率 $=\frac{Progress}{B}$ の、同じ四半期・同じ物差しの過去の開示（2016-10〜）での順位 × 10（例年並み ≈ 5）。$B$ = 前年同期の進捗（Q×12.5% 未満なら Q×12.5%、前年が無ければ Q×25%） | 過去の99%より速いペース |
...
| 軸8 進捗期待 | $\mathrm{clamp}(0,10,\ 5.0+\frac{Progress - Quarter\times25}{2})$ | **基準 = 前年の同じ四半期の進捗**（前年のその四半期までの累計営業利益 ÷ 前年の通期実績）。前年同期の進捗が Q×12.5% 未満なら Q×12.5%、前年の数字が無い・0以下・決算期の変更なら Q×25%。**比率 = 進捗率 ÷ 基準**を、同じ四半期・同じ物差しの過去の開示（[`research/progress_percentiles.py`](research/progress_percentiles.py) で作った点数表）の中での順位で点数にする。本決算（4Q）は `null`。表に基準と比率を表示 | 下期に利益が偏る会社は、例年どおりでも Q×25% に届かず常に「遅れ」に見えた。固定の幅だと第1四半期の多くが 0点か10点に張り付いた（ウォッチリスト43銘柄中19銘柄 → 1銘柄）。「通期実績が会社予想を上回るか」の見分け（AUC）は Q×25% の比率 0.705 → 0.739（約6.8万件。2017〜2026 の10年中9年で上回り、下回ったのは 2021年だけ。前年＝2020年度の四半期の偏りがコロナで例年と違ったためと推測）。2026-09-25 に運用者の選択で変更 |
...
| 株価推移とタイムラインの期間 | 記述なし（実装は直近1年） | **目的変数と同じ78週（368営業日 + 当日）**。見出しの週数は予測モデルと同じ換算（245営業日 = 52週）で日足の本数から出す（368営業日は暦では約550日 = 78.6週）。株価推移には出来事（決算・78週高値の更新・出来高急増）の日に**縦の破線**を引き、点に触れるとその日の出来事を出す | 予測モデルが見る窓と同じ期間で値動きを見るため。出来事がチャートのどこに当たるかを一目で分かるようにするため（2026-09-26 に運用者の依頼で変更） |
