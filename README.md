...
| 株価推移とタイムラインの期間 | 記述なし（実装は直近1年） | **目的変数と同じ78週（368営業日 + 当日）**。株価推移には出来事（決算・78週高値の更新・出来高急増）の日に**縦の破線**を引き、点に触れるとその日の出来事を出す | 予測モデルが見る窓と同じ期間で値動きを見るため。出来事がチャートのどこに当たるかを一目で分かるようにするため（2026-09-26 に運用者の依頼で変更） |
