...
| 3 | ROE | ROE（自己資本利益率） | `S(x, 5, 25)` | 25% 以上 |
| 4 | 営業利益率 | 営業利益率 | `S(x, 0, 20)` | 20% 以上 |
| 5 | テクニカル | 78週高値接近率 $R$（368営業日） | $R\ge98$→10.0 ／ $R\ge90$→$8.0+\frac{R-90}{8}\times1.5$ ／ $R\ge80$→$6.0+\frac{R-80}{10}\times2.0$ | 98% 以上 |
| 6 | 出来高 | 出来高モメンタム × 機関参入度 | $\mathrm{clamp}(0,10,\ 5.0+\frac{Trend-100}{100}\times5.0)\times decay$ | 売買代金10億円以上かつ増 |
| 7 | 信用倍率 | 信用倍率 $C$ | $C\le1$→10.0 ／ $C\le3$→$10.0-\frac{C-1}{2}\times3.0$ ／ $C\le10$→$7.0-\frac{C-3}{7}\times5.0$ | 1.0倍以下 |
...
| 軸7 信用倍率 $C>10$ | 未定義 | $\max(0,\ 2.0 - \frac{C-10}{10}\times2.0)$（20倍で 0） | 同上 |
...
| 直近4四半期（TTM） | 記述なし | **会計期間の順で連続した4四半期**。1つでも欠ければ TTM は作らない | 開示日の順で最後の4行を取ると、過去の決算を後から出し直した銘柄で古い四半期が紛れ込む（2026-09-25 に 9081 で発生） |
| ROE | 「ROE」とのみ | **TTM純利益 / 自己資本**を優先。4四半期が揃わないときだけ J-Quants の提供値（**小数**なので % に直す） | 提供値は本決算の行にだけあり、先に使うと本決算の直後だけ定義が変わる。時点どうし・銘柄どうしを同じ物差しで比べるため |
| 前年同期比 | 記述なし | 前年同期の値が **0以下なら成長率を `null`** とする | 赤字→黒字転換を「+1000%」等と表示すると誤解を招くため |
| 軸5 テクニカルの高値の窓 | 「52週高値接近率」 | **78週（368営業日）**。予測モデルの HIGH_WINDOW と同じ窓・同じ条件（分割調整後の高値の最大。368営業日の履歴が無い銘柄と、窓の半分以上で高値が無い銘柄は `null`）。ゾーン判定とタイムラインの高値更新も同じ窓 | 予測モデルがブレイクと判定するのと同じ物差しで画面を見るため（2026-09-25 に運用者の指示で変更）。予測タブから送った銘柄（78週）と J-Quants から取った銘柄（52週）が同じ画面に混ざっていたのも解消 |
| タイムマシーンの過去時点 | 「6ヶ月前 / 3ヶ月前 / 現在」 | **その日までに開示・公表済みのデータのみ**で、現在と同じ式で再計算（先読みなし）。決算は開示日で切ってから四半期の表を作り（後日の訂正に置き換えない）、週次の信用残は公表日（基準日の翌週の第2営業日）で切る | 過去時点で実際に見えていた情報だけで判断を検証するため |
...
| `/markets/margin-interest` | `/markets/weekly_margin_interest` | 信用倍率（8軸の信用倍率） |
